...
module Admin
  class LoanRequestSerializer < BaseSerializer
    ...
    attribute :edit_path do |loan_request|
      url_helpers.admin_loan_request_edit_path(id: loan_request.id)
    end

    attribute :show_path do |loan_request|
      url_helpers.admin_loan_request_show_path(id: loan_request.id)
    end
    ...
    attribute :user_full_name do |loan_request|
      loan_request.user.profile.full_name
    end

    attribute :user_email do |loan_request|
      loan_request.user.email
    end

    attribute :user_office_phone do |loan_request|
      loan_request.user.profile.office_phone
    end

    attribute :user_cellular_phone do |loan_request|
      loan_request.user.profile.cellular_phone
    end
  end
end
