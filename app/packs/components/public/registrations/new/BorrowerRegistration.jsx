...
function New() {
  ...
  const onSubmit = createHandleSubmit({
    handleSubmit: (values) => {
      return dispatch(createUser({ ...values, user_type: "borrower" }));
    },
    handleSuccess: () => {
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-8 sm:px-6 lg:px-8">
          ...
          <Formik {...{ initialValues, validationSchema, onSubmit }}>
            <Form className="space-y-6">
              ...
              <div>
                <button
                  type="submit"
                  className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-congress-blue-600 hover:bg-congress-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-congress-blue-500"
                >
                  Create account
                </button>
              </div>
      ...
      <div className="max-w-7xl mx-auto text-center py-8 px-4 sm:px-6 lg:py-16 lg:px-8">
        <h2 className="text-center text-2xl font-extrabold text-gray-900">
          <span className="block">Not a borrower?</span>
