...
export const validationSchema = yup.object().shape({
  ...
  locationCity: yup.string().required("Can't be blank.").nullable(),
  locationState: yup
    .object()
    .shape({
      label: yup.string(),
  ...
  locationPostalCode: yup
    ...
    .trim()
    .nullable()
    .matches(/(^\d{5}$)/, "Is not in correct format, example: 90210")
    .required("Can't be blank."),
  borrowerName: yup.string().required("Can't be blank.").nullable(),
  borrowerMobileNumber: yup
    .string()
    .required("Can't be blank.")
    .trim()
    .nullable()
    .matches(
      /^[(]?\d{3}[)]?[(\s)?.-]?\d{3}[\s.-]?\d{4}$/,
      "Is not in correct format, example: 555-867-5309"
    ),
  borrowerEmail: yup
    .string()
    .required("Can't be blank.")
    ...
    .matches(
      ...
      "Is not in correct format, example: fred@example.com"
    ),
  propertyType: yup
    .object()
    .shape({
    ...
    .nullable(),
  unitQuantity: yup.number().nullable(),
  netRentableSquareFeet: yup.number().nullable(),
  physicalOccupancy: yup.number().nullable(),
  yearBuilt: yup.number().nullable(),
  propertyComments: yup
    .string()
    .trim()
    .test(
      "len",
      "Must be less than 2,000 characters",
      (val) => !val || (val && val.length <= 2000)
    ),
  transactionType: yup.string().required("Can't be blank.").trim().nullable(),
  purchasePrice: yup.number().nullable(),
  purchaseUnderContract: yup
    .object()
    .nullable()
    .when("transactionType", {
      is: "Purchase",
      then: yup.object().required("Can't be blank."),
    }),
  purchaseClosingDate: yup.string().trim().nullable(),
  refinancePropertyValue: yup.number().nullable(),
  refinanceOutstandingLoanBalance: yup
    .number()
    .nullable()
    .when("transactionType", {
      is: "Refinance",
      then: yup.number().required("Can't be blank.").nullable(),
    }),
  numberKeyPrincipals: yup.number().nullable(),
  sponsorCombinedLiquidity: yup.number().nullable(),
  sponsorCombinedNetWorth: yup.number().nullable(),
  sponsorCombinedYearsExperience: yup.number().nullable(),
  sponsorCreditIssuesDisclosed: yup.string().nullable(),
  sponsorComments: yup
    .string()
    .nullable()
    .trim()
