...
function Edit({ loanRequestId }) {
  ...
  const providedValues = loanRequest.attributes;
  ...
  providedValues.loanAmountRequested = loanRequest.attributes.loanAmountCents / 100.0
  ...
  providedValues.propertyType = toSelectOption(providedValues.propertyType);
  providedValues.locationState = toSelectOption(providedValues.locationState);
  providedValues.borrowerState = toSelectOption(providedValues.borrowerState);
  providedValues.locationPostalCode = providedValues.postalCode

  if (providedValues.transactionType === "Purchase") {
    providedValues.purchaseUnderContract = find(foreignNationalsConsideredOptions, {
      value: providedValues.purchaseUnderContract ? "Yes" : "No",
    });
  }
        ...
        <main>
          ...
          <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <main>
              {/* Page header */}
              <div className="mt-8 ">
                <LoanRequestForm {...{ providedValues }} />
              </div>
            </main>
          </div>
        </main>
